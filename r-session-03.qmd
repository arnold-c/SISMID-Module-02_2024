...
In terms of years, $\text{D} = \frac{14}{365} \approx 0.0384$, and the recovery rate is the inverse i.e., $\gamma = \frac{365}{14}$.
...
```{r}
sse_optim_params_g <- c(beta = log(0.055), gamma = log(365/14), S_init = log(5000), I_init = log(1))
...
::: {.callout-important}
It is always worth performing a sanity check on the best fit predictions.
For example, as we have now estimated gamma (the recovery rate), which has a natural interpretation as the inverse of the duration of infection, we can see if the predicted values make sense.
Does it make sense that `gamma` is different in each site?
...
Does it make sense that the duration of infection is only 1-5 days?
In situations like this, it would make sense to put constraints on the parameter space that is being optimized over, but that is beyond the scope of this tutorial.
:::
...
#| echo: false
#| eval: false
closed.sir.model.g <- function (t, x, params) {  #SIR model equations
  X <- x[1]
  Y <- x[2]
  beta <- params[1]
  gamma <- params[2]
...
sse.sir.g <- function(params0,data,site){  #function to calculate squared errors
  ...
  out <- as.data.frame(ode(c(X=X0,Y=Y0),times=t,closed.sir.model.g,c(beta, gamma), hmax=1/120))
...
mod.pred.g1<-as.data.frame(ode(c(X=exp(fit1.g$par[2]),Y=exp(fit1.g$par[3])),times=t,
closed.sir.model.g,c(exp(fit1.g$par[1]),exp(fit1.g$par[4])),hmax=1/120))
...
mod.pred.g2<-as.data.frame(ode(c(X=exp(fit2.g$par[2]),Y=exp(fit2.g$par[3])),times=t,
closed.sir.model.g,c(exp(fit2.g$par[1]),exp(fit2.g$par[4])),hmax=1/120))
#obtain model predictions
...
mod.pred.g3<-as.data.frame(ode(c(X=exp(fit3.g$par[2]),Y=exp(fit3.g$par[3])),times=t,
closed.sir.model.g,c(exp(fit3.g$par[1]),exp(fit3.g$par[4])),hmax=1/120))
...
#| echo: false
#| eval: false
sse_optim_params_S0 <- c(beta = log(0.055), gamma = log(365/14), I_init = log(1))
...
#| echo: false
#| eval: false
sse_optim_params_I0 <- c(beta = log(0.055), gamma = log(365/14), S_init = log(5000), I_init = log(1))
...
#| echo: false
#| eval: false
sse_optim_params_S0_I0 <- c(beta = log(0.055), gamma = log(365/14), S_init = log(5000), I_init = log(1))
...
#| echo: false
#| eval: false
sse.sir.x0 <- function(params0,data,site){  #function to calculate squared errors
  data<-data[data$site==site,]    #working dataset, based on site
  t <- data[,1]*14/365            #time in biweeks
  cases <- data[,3]               #number of cases
  beta <- exp(params0[1])            #parameter beta
...
sse.sir.y0 <- function(params0,data,site){  #function to calculate squared errors
  data<-data[data$site==site,]    #working dataset, based on site
  t <- data[,1]*14/365            #time in biweeks
  cases <- data[,3]               #number of cases
  beta <- exp(params0[1])            #parameter beta
...
sse.sir.x0y0 <- function(params0,data,site){  #function to calculate squared errors
  data<-data[data$site==site,]    #working dataset, based on site
  t <- data[,1]*14/365            #time in biweeks
  cases <- data[,3]               #number of cases
  beta <- exp(params0[1])            #parameter beta
...
fit1.x0y0 <- optim(params0,sse.sir.x0y0,data=niamey,site=1, hessian=TRUE) #fit
